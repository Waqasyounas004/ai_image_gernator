...
def enhance_prompt(user_prompt: str, fallback_on_failure: bool = True) -> str:
    ...
    if fallback_on_failure:
        print(f"[Warning] Groq API call failed across all candidate models: {last_exception}. Using original prompt as fallback.")
        return user_prompt.strip()
    else:
        raise RuntimeError(f"Groq API call failed: {last_exception}") from last_exception
